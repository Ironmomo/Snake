...
# Agent
class Double_DQN_Trainer_Agent(Agent):

    def __init__(self, env: Environment, policy_model: DQN, target_model: DQN, visualize: bool = False, buffer_capacity: int = 10000, batch_size: int = 125, lr: int = 0.1, c: int = 125, max_steps: int = 2000, gamma: float = 0.9, eps_start: float = 0.1, eps_end: float = 0.001, decay_rate: int = 1000):
        super().__init__(env)
    ...
    def __update(self, cur_step: int):
        ...
        next_state_val_tensor[non_final_mask] = self.target_model(non_final_next_state_tensor).max(1).values
